...
function saveTask(taskNameValue, taskPriorityValue, taskTypeValue, taskRelatedNotificationValue,
    ...
    var getMethodUrl = document.URL.substring(0, document.URL.indexOf("/tasks/add-task"));
    getMethodUrl += document.getElementById("urlToSaveTask").getAttribute("href");
    ...
    xhr.onreadystatechange = function(){
        if (xhr.readyState == 4 && xhr.status == 200){
            window.location.href = document.URL.substring(0, document.URL.indexOf("/tasks/add-task")) + "/tasks/current-tasks-list";
        }
    };
    xhr.send();
}